function [FluxPotentials_2d,FluxPotential_solutions_2d] = FPA2_cluster_simple_decay_eng(model,targetRxns,master_expression,distMat,labels,nSeq,manualPenalty,manualDist,maxDist,blockList, constantPenalty,parforFlag,penalty_defined,alpha, distMat_unweighted,labels_unweighted, eng_suffix)
...
if parforFlag
    for nInd = 1:length(nSeq)
        ...
        parfor i = 1:length(targetRxns)
            ...
            for j = 1:size(penalty,2)
                ...
                if doForward 
                    ...
                    % we implement simple decay here: 
                    % weight = 0 if dist > n
                    % weight = 1*penalty if dist <= n
                    subDist = distMat(strcmp(labels, [myrxn,'_f']),:);
                    pDist_f = ones(1, length(subDist));
                    pDist_f(subDist > n) = 0;    
                ...
                if doReverse 
                    ...
                    % weight = 0 if dist > n
                    % weight = 1*penalty if dist <= n
                    subDist = distMat(strcmp(labels, [myrxn,'_r']),:);
                    pDist_r = ones(1, length(subDist));
                    pDist_r(subDist > n) = 0;  
        ...
        if length(nSeq) > 1
            ...
            save([eng_suffix,'/simpleDecay_','_n_',num2str(n),'.mat'],'FluxPotential_solutions')
        else
