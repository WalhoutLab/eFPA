function [allowance, solution] = GetMinAllowance_COMPASS(model,w, w_label, obj_rxn,default_w, Vmax) 
...
model.lb(strcmp(model.rxns,obj_rxn)) = Vmax.f .* 0.95;
...
if solution.stat == 1
    allowance = solution.f;
    Vmax = solution.x(strcmp(model.rxns,obj_rxn));
else
    allowance = NaN;
    Vmax = NaN;
    solution = {''};
