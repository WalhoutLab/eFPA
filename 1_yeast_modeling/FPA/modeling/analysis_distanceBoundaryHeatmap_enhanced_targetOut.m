...
%% load the control2 - base 2 boundary 6 - target out
...
fdr_r = mafdr(p_FPA_targetOut,'BHFDR', true);% BHFDR adjustment is chosen to keep strigency (control FDR instead of estimate FDR (which is used in pFDR(qvalue)))
fprintf('%d rxns give significant positive correlation by pearson\n',sum(r_FPA_targetOut(fdr_r<0.05)>0));

control_r_3 = r_FPA_targetOut;
control_p_3 = p_FPA_targetOut;
control_testedRxns_3 = testedRxn_FPA_targetOut;
control_cv_3 = deltaminmax;

%% make the heatmap 
load(['output/Titration_relativeExp_wtdDist_expDecay_FineGrained_targetOut.mat'])
...
[A B] = ismember(targetRxns,control_testedRxns_3);
ctrVp3 = ones(length(targetRxns),1);
ctrVr3 = zeros(length(targetRxns),1);
ctrVcv3 = zeros(length(targetRxns),1);
ctrVp3(A) = control_p_3(B(A));
ctrVr3(A) = control_r_3(B(A));
ctrVcv3(A) = control_cv_3(B(A));

pMat_valid = [ctrVp3, ctrVp2, ctrVp, pMat];
rMat_valid = [ctrVr3, ctrVr2, ctrVr, rMat];
CVmat_valid = [ctrVcv3, ctrVcv2, ctrVcv, CVmat];
...
% bin the r according to converted distance 
dorders = 0:40;
...
for i = 1:(length(dorders)-1)
    ...
    for j = 1:size(rMat_valid_normalized_binned,1)
        ...
        if any(pass)
            ...
            sigMat_binned(j,i+2) = any(pMat_valid(j,pass) < 0.05 & rMat_valid(j,pass) > 0 & CVmat_valid(j,pass) > 0.2);
        end
    end
end
...
writetable(t,'output/relCorr_heatmapTbl_realDist_targetOut.csv','WriteRowNames',1);

IDs = [{'base 2 - boundary 6 - target out','base 2 - boundary 6','expression only'},strsplit(num2str(dorders))];
boundaries =  cell2table(IDs');
writetable(boundaries,'output/heatmapTbl_boundaries_realDist_targetOut.csv');

t = array2table(sigMat_binned);
t.Properties.RowNames = targetRxns_valid;
writetable(t,'output/heatmapTbl_sigLabel_realDist_targetOut.csv','WriteRowNames',1);
