...
plt.LegendLoc = 'eastoutside';
plt.FontSize = 7;
plt.export(['figures/GST_example/FPA_GST_mechanism_',targetRxns{i},'.pdf']);
...
%% plot the expression vs. flux of the ROI
rxnID = 'r_0892'; % we also ploted r_4214 r_0486 r_0892
...
if any(strcmp(rxnID,valid_rxns_pro_perPro)) 
    ...
    plt.YMinorTick = 'off';
    plt.TickDir = 'out';
    plt.export(['figures/GST_example/FPA_GST_mechanism_expression_',rxnID,'_flux_',targetRxn,'.pdf']);
else
    fprintf('not measured!\n')
end
%% ROI target expression vs flux 
rxnID = 'r_0485';
figure;
myLevel = normalizedLevel_pro_perPro(strcmp(valid_rxns_pro_perPro,rxnID),:);
...
plt.XLim = xlim1;
plt.YLim = ylim1;
plt.export(['figures/GST_example/FPA_GST_mechanism_exp2flux_',rxnID,'.pdf']);
%% plot ROI rFP vs flux
targetRxn = 'r_0485';
dist = 18.5;
...
plt.XLim = xlim1;
plt.YLim = ylim1;
plt.export(['figures/GST_example/FPA_GST_flux_rFP_',targetRxn,'_d_',num2str(dist),'.pdf']);
...
%% plot the robustness analysis curve
figure
hold on
for i = 1:25
...
plt.TickDir = 'out';
plt.Interpreter = 'None';
plt.export(['figures/GST_example/FPA_GST_glutathione synthesis_flux_robustness.pdf']);