...
plot(dorders_wtdDist_expDecay100(1:14),N_sigCorr_highCV_wtdDist_expDecay100(1:14) ,'o-','LineWidth',2,'Color','#EDB120','MarkerSize', 3)
...
ylabel('Number of significantly correlated reactions ');
ylim([20 80])
legend({'improved FPA (base = 2)','improved FPA (base = 100)','original FPA + weighted distance', 'original FPA','expression only'},'FontSize',7);
plt = Plot(); % create a Plot object and grab the current figure
...
% plot(r_exp(r_FPA>0.3 & r_FPA - r_exp > 0.3 ),r_FPA(r_FPA>0.3 & (r_FPA - r_exp) > 0.3),'r.')
sig1 = p_exp_only<0.05 & r_exp_only >0;
sig2 = p_FPA_targetOut<0.05 & r_FPA_targetOut >0 & deltaminmax > 0.2;
...
plt.YMinorTick = 'off';
plt.TickDir = 'out';
plt.export(['figures/r_comparison_FPAtargetOut_expressionOnly.pdf']);
...
% plot(r_exp(r_exp>0.3 & r_exp - r_FPA > 0.3 ),r_FPA(r_exp>0.3 & (r_exp - r_FPA) > 0.3),'b.')
% plot(r_exp(r_FPA>0.3 & r_FPA - r_exp > 0.3 ),r_FPA(r_FPA>0.3 & (r_FPA - r_exp) > 0.3),'r.')
sig1 = p_FPA_ori<0.05 & r_FPA_ori >0 & deltaminmax_FPA_ori > 0.2;
sig2 = p_FPA_targetOut_ori<0.05 & r_FPA_targetOut_ori >0 & deltaminmax_ori > 0.2;
