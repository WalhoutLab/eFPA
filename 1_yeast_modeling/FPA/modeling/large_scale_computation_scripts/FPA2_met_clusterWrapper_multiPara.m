function [FluxPotentials_2d] = FPA2_met_clusterWrapper_multiPara(model,targetRxns,master_expression,distMat,distMat_raw,labels,nSeq,manualPenalty,manualDist,maxDist,blockList, constantPenalty,parforFlag,penalty_defined,alpha, base)
...
save([tmpDir,'/variables.mat'],'model','master_expression','distMat','distMat_raw','labels','nSeq','manualDist','maxDist','blockList', 'constantPenalty','parforFlag','penalty','targetRxns','environment','base','alpha','-v7.3','-nocompression');
...
for i = 1:length(splits)-1
    ...
    cmd = ['matlab -nodisplay -nosplash -nojvm -r \"FPA2_met_clusterExecuter ',tmpDir,' ',num2str(splits(i)),' ',num2str(splits(i+1)-1),' ',batchID,'\"'];
...
batchID = ['batch_',num2str(i)];
env_cmd = 'module load gurobi/900 && module load matlab/R2019a && ';
cmd = ['matlab -nodisplay -nosplash -nojvm -r \"FPA2_met_clusterExecuter ',tmpDir,' ',num2str(splits(i)),' ',num2str(length(targetRxns)),' ',batchID,'\"'];
full_cmd = [env_cmd, cmd, ' > ',tmpDir,'/',batchID,'.log && wait'];
bsub_cmd = ['bsub -q short -W 1:00 -n 1 -R rusage[mem=4096] -e ',tmpDir,'/err_',batchID,'.log ','-J ',batchID,' "'];
cmd_ready = [bsub_cmd, full_cmd,'"'];
system(cmd_ready);
