function [FluxPotentials_2d] = FPA2_cluster_simpleDecay_highIO(model,targetRxns,master_expression,distMat,labels,nSeq,manualPenalty,manualDist,maxDist,blockList, constantPenalty,parforFlag,penalty_defined,alpha)
...
npool = 10; % in the current loop structure, npool should be less than #conditions,so < 25
...
if ~parforFlag % the high IO version is designed for very intensive randomization that uses cluster; no parfor support!
    ...
    FluxPotentials_2d{1} = FluxPotentials_all;
else 
    error('discontinued!')
end
