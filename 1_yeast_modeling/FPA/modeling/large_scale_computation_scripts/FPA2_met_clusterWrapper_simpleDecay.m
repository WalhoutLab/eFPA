function [FluxPotentials_2d] = FPA2_met_clusterWrapper_simpleDecay(model,targetRxns,master_expression,distMat,distMat_raw, labels,nSeq,manualPenalty,manualDist,maxDist,blockList, constantPenalty,parforFlag,penalty_defined,alpha)
...
save([tmpDir,'/variables.mat'],'model','master_expression','distMat','distMat_raw','labels','nSeq','manualDist','maxDist','blockList', 'constantPenalty','parforFlag','penalty','targetRxns','environment','alpha','-v7.3','-nocompression');
...
for i = 1:length(splits)-1
    ...
    cmd = ['matlab -nodisplay -nosplash -nojvm -r \"FPA2_met_clusterExecuter_simpleDecay ',tmpDir,' ',num2str(splits(i)),' ',num2str(splits(i+1)-1),' ',batchID,'\"'];
    full_cmd = [env_cmd, cmd, ' > ',tmpDir,'/',batchID,'.log && wait'];
    bsub_cmd = ['bsub -q short -W 1:00 -n 1 -R rusage[mem=4096] -e ',tmpDir,'/err_',batchID,'.log ','-J ',batchID,' "'];
...
batchID = ['b_',uid,'_',num2str(i)];
env_cmd = 'module load gurobi/900 && module load matlab/R2019a && ';
cmd = ['matlab -nodisplay -nosplash -nojvm -r \"FPA2_met_clusterExecuter_simpleDecay ',tmpDir,' ',num2str(splits(i)),' ',num2str(length(targetRxns)),' ',batchID,'\"'];
full_cmd = [env_cmd, cmd, ' > ',tmpDir,'/',batchID,'.log && wait'];
bsub_cmd = ['bsub -q short -W 1:00 -n 1 -R rusage[mem=4096] -e ',tmpDir,'/err_',batchID,'.log ','-J ',batchID,' "'];
cmd_ready = [bsub_cmd, full_cmd,'"'];
system(cmd_ready);
...
pause(30); % wait for all jobs to be submitted
...
while ~isempty(runningBatches)
    ...
    if any(failedBatches == length(splits)) % if last batch needs to be rerun
        ...
        cmd = ['matlab -nodisplay -nosplash -nojvm -r \"FPA2_met_clusterExecuter_simpleDecay ',tmpDir,' ',num2str(splits(i)),' ',num2str(length(targetRxns)),' ',batchID,'\"'];
        full_cmd = [env_cmd, cmd, ' > ',tmpDir,'/',batchID,'.log && wait'];
        bsub_cmd = ['bsub -q short -W 1:00 -n 1 -R rusage[mem=10000] -e ',tmpDir,'/err_',batchID,'.log ','-J ',batchID,' "'];
        cmd_ready = [bsub_cmd, full_cmd,'"'];
        system(cmd_ready);
    ...
    fprintf('job monitoring round finished; %d batches remain unfinished; checkpoint interval is 30 sec...\n',length(runningBatches));
    pause(30);
end
...
for i = 2:length(splits)
    ...
    for nInd = 1:length(nSeq) 
        FluxPotentials_2d{bInd}{nInd} = [FluxPotentials_2d{bInd}{nInd};FP{bInd}{nInd}];
    end
end
system(['rm -rf ',tmpDir]);
% delete([tmpDir,'/*']);
