...
seed = 1126;
rng(seed);
mkdir(['output/randomization/seed_',num2str(seed),'/'])
...
for c = 1:(nsample+1)
    ...
    save(['output/randomization/seed_',num2str(seed),'/rand_',num2str(c),'_default_FPA.mat'],'FP_collection_2','targetRxns');

    [FP_collection_2] = FPA2_clusterWrapper_multiPara(model,targetRxns,{},distMat_wtd,labels,n2, manualPenalty,{},max(distMat_wtd(~isinf(distMat_wtd))),blocklist, {},false,penalty_pro,alpha,base);
    save(['output/randomization/seed_',num2str(seed),'/rand_',num2str(c),'_flexi_FPA.mat'],'FP_collection_2','targetRxns');
end
