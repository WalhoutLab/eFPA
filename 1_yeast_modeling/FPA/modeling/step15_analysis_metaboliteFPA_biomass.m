...
% metTbl = readtable('./../input/YeastJoshua/originalDataTbl/biomass_component_table.xlsx');
% metMat = [];
% % make the matched matrix
% for i = 1: length(conditions)
%     metMat(:,i) = metTbl.(conditions{i});
% end
% metLabel = metTbl.MetaboliteModelName;
...
targetRxns_UID = regexprep(targetRxns,'^NewMet_','');
...
valid_mets_prod = targetRxns_UID(~rmInd1);
relFP_cons = relFP_cons(~rmInd2,:);
valid_mets_cons = targetRxns_UID(~rmInd2);
...
for j = 1:length(metLabel)
    ...
    if any(strcmp(valid_mets_prod,metLabel{j}))
        ...
        [r(end+1),p_r(end+1)] = corr(unique(relFP_prod(strcmp(valid_mets_prod,metLabel{j}),:),"rows")',metMeasure','type','Pearson');
        deltaminmax(end+1) = max(unique(relFP_prod(strcmp(valid_mets_prod,metLabel{j}),:),"rows")) - min(unique(relFP_prod(strcmp(valid_mets_prod,metLabel{j}),:),"rows"));
    end
end
...
for j = 1:length(metLabel)
    ...
    if any(strcmp(myset,metLabel{j}))
        lm = fitlm(unique(relFP_prod(strcmp(valid_mets_prod,metLabel{j}),:),'rows')',metMeasure');
        figure;
        plot(lm)
        title(metLabel{j})
    end
...
%         testedMet(end+1) = metLabel(j);
%         [r(end+1),p_r(end+1)] = corr(unique(relFP_cons(strcmp(valid_mets_cons,metLabel{j}),:),"rows")',metMeasure','type','Pearson');
%         deltaminmax(end+1) = max(unique(relFP_cons(strcmp(valid_mets_cons,metLabel{j}),:),"rows")) - min(unique(relFP_cons(strcmp(valid_mets_cons,metLabel{j}),:),"rows"));
%     end
% end
% fdr_r = mafdr(p_r,'BHFDR', true);% BHFDR adjustment is chosen to keep strigency (control FDR instead of estimate FDR (which is used in pFDR(qvalue)))
% fprintf('%d mets give significant positive correlation by pearson\n',sum(r(fdr_r<0.05)>0));
...
%     metMeasure = metMat(j,:);
%     if any(strcmp(myset,metLabel{j}))
%         lm = fitlm(relFP_cons(strcmp(valid_mets_cons,metLabel{j}),:)',metMeasure');
%         figure;
%         plot(lm)
%         title(metLabel{j})
%     end
...
dorders = 0:0.5:40;
rMat_prod = zeros(length(metLabel),length(dorders));
FDRmat_prod = ones(length(metLabel),length(dorders));
pMat_prod = ones(length(metLabel),length(dorders));
CVmat_prod = ones(length(metLabel),length(dorders));

rMat_cons = zeros(length(metLabel),length(dorders));
FDRmat_cons = ones(length(metLabel),length(dorders));
pMat_cons = ones(length(metLabel),length(dorders));
CVmat_cons = ones(length(metLabel),length(dorders));
for nn = 1: length(dorders)
    ...
    [A B] = ismember(testedMet,metLabel);
    rMat_prod(B(A),nn) = r;
...
% default FPA
[A B] = ismember(metLabel,localFPA_testedMets_prod);
ctrVp2 = ones(length(metLabel),1);
ctrVr2 = zeros(length(metLabel),1);
ctrVdeltaminmax2 = zeros(length(metLabel),1);
...
t1.Properties.RowNames = metLabel;
...
% writetable(t2,'output/PCC_titration_consumingPotential2.csv','WriteRowNames',1);
...
targetRxns(r_max_sig<0.05 & r_max >0)
...
% plt.LegendLoc = 'NorthWest';
% plt.FontName = 'Arial';
% plt.export('figures/optimal_boundary_FPA_metabolite_correlation_consumingPotential2.pdf');
% targetRxns(r_max_sig<0.05 & r_max >0)

%% filter the PCC matrix and only keep the predicted reactions 
t = struct();
t.rxn1 = metLabel(any(FDRmat_prod < 0.05 & rMat_prod > 0 & CVmat_prod > 0.2,2));
t = struct2table(t);
writetable(t,'output/producing_sig_rxns.csv');
%% convert matrux 
keep = any(FDRmat_prod < 0.05 & rMat_prod > 0 & CVmat_prod > 0.2,2);

rMat_valid = rMat_producing(keep,:);
pMat_valid = pMat_producing(keep,:);
targetRxns_valid = metLabel(keep);
deltaminmaxmat_valid = deltaminmaxmat_producing(keep,:);

% row-wise normalization
rMat_valid_normalized = rMat_valid ./ max(rMat_valid,[],2); % relative r 
rMat_valid_normalized_ori = rMat_valid_normalized;

%% make distance conversion matrix
%% 1. load the model and prepare the model
addpath('./../scripts/')
model = loadYeatModel();
% phosphate exchange
...
cvtDistMat = nan(length(metLabel),length(dorders));
maxDist = max(distMat_normal(~isinf(distMat_normal)));
for zz = 1:length(dorders)    
    %% convert the distance to real distance 
    for i = 1:length(metLabel)
        theMet = model.mets(strcmp(model.metNames, metLabel{i}));
        candidateRxns = model.rxns(logical(model.S(strcmp(model.mets,theMet),:)));
        % add direction label
        for j = 1:length(candidateRxns)
            if model.S(strcmp(model.mets,theMet),strcmp(model.rxns,candidateRxns{j})) > 0% only the producing direction is considered
                candidateRxns{j} = [candidateRxns{j},'_f'];
            else
                candidateRxns{j} = [candidateRxns{j},'_r'];
            end
        end
        ...
        if ~isempty(candidateRxns)
            for k = 1:length(candidateRxns)
                rxnSet = labels(distMat_wtd(strcmp(labels,candidateRxns{k}),:)+1 <= dorders(zz));% these reactions were (expected to be) scaned through in the integration 
                if ~isempty(rxnSet)    
                    myMax(k) = max(distMat_normal(strcmp(labels,candidateRxns{k}),ismember(labels,rxnSet)))+1; % the maximum distance for those scaned rxns
                else
                    myMax(k) = 0;   
                end
            end
            cvtDistMat(i,zz) = max(myMax);
        else
...
rMat_valid_normalized_binned = nan(size(rMat_valid_normalized,1),length(dorders)+1);
for i = 1:(length(dorders)-1)
    lb = dorders(i);
    ub = dorders(i+1);
    for j = 1:size(rMat_valid_normalized_binned,1)
        pass = find(cvtDistMat(j,:) >= lb & cvtDistMat(j,:) < ub)+1;
        if any(pass)
            rMat_valid_normalized_binned(j,i+1) = max(rMat_valid_normalized(j,pass));
        end
    end
end
rMat_valid_normalized_binned(:,1) = rMat_valid_normalized(:,1);
% save data 
t = array2table(rMat_valid_normalized_binned);
t.Properties.RowNames = targetRxns_valid;
writetable(t,'output/relCorr_heatmapTbl_realDist_biomass_met.csv','WriteRowNames',1);

IDs = [{'base 2 - boundary 6'},strsplit(num2str(dorders))];
boundaries =  cell2table(IDs');
writetable(boundaries,'output/heatmapTbl_boundaries_realDist_biomass_met.csv');
