...
% bin the r according to real distance 
load('output/mmIRS.mat');
mmIRS = mmIRS(keep,:);
rMat_valid_normalized_binned = nan(size(rMat_valid_normalized,1),size(rMat_valid_normalized,2));
for i = 1:(length(dorders)-1)
    lb = dorders(i);
    ub = dorders(i+1);
    for j = 1:size(rMat_valid_normalized_binned,1)
        pass = mmIRS(j,:) >= lb & mmIRS(j,:) < ub;
        if any(pass)
...
%% plot the scarttering - skip
figure;
hold on 
for i = 1:size(rMat_valid_normalized,1)
...
%% save data 
t = array2table(rMat_valid_normalized_ori);
t.Properties.RowNames = targetRxns_valid;
writetable(t,'output/relCorr_heatmapTbl_wtdDist.csv','WriteRowNames',1);

t = array2table(rMat_valid_normalized);
t.Properties.RowNames = targetRxns_valid;
writetable(t,'output/relCorr_heatmapTbl_BinnedRealDist.csv','WriteRowNames',1);
...
t = array2table(mmIRSmat);
t.Properties.RowNames = targetRxns_valid;
writetable(t,'output/mmIRS_wtdDist.csv','WriteRowNames',1);

%% bin by real distance - replace x-axis by real distance measures 
