%% About 
% the casestudy of prediction mechanisms of glycan synthesis flux
%% load data
...
rxnInfo(ismember(rxnInfo(:,1),{'r_4235','r_0770','r_0773'}),4) = {'Other energy metabolism'};
rxnInfo(ismember(rxnInfo(:,1),{'r_1714','r_1811','r_1654','r_1166','r_2034','r_1115'}),4) = {'Transport reactions'};

% reshape the matrix to make the subsystem-contribution matrix
% we only keep a few top subsystems in the plot
subsys = {'Gluconeogenesis','Oxidative phosphorylation','Transport reactions','Other energy metabolism','Citrate cycle (TCA cycle)','Glycine, serine and threonine metabolism','Others'};
...
% rename Gluconeogenesis subsystem
...
plt.LegendLoc = 'eastoutside';
plt.FontSize = 7;
plt.export(['figures/glycan_example/FPA_glycan_mechanism_',targetRxns{i},'.pdf']);
...
% subset 
glycolysisSet = rxnInfo(strcmp(rxnInfo(:,4),'Gluconeogenesis'),:);
%% plot the expression vs. flux for the glycolysis subset rxns
for j = 1:size(glycolysisSet,1)
    rxnID = glycolysisSet{j,1};
    ...
    if any(strcmp(rxnID,valid_rxns_pro_perPro)) 
        ...
        plt.YMinorTick = 'off';
        plt.TickDir = 'out';
        plt.export(['figures/glycan_example/FPA_glycan_mechanism_expression_',rxnID,'_flux_',targetRxn,'.pdf']);
    else
        fprintf('not measured!\n')
    end
end
%% plot rFP vs flux of the ROI
targetRxn = 'r_0362';
rxnID = 'r_0362';
dist = 25.5;
...
plt.XLim = xlim1;
plt.YLim = ylim1;
plt.export(['figures/glycan_example/FPA_glycan_flux_rFP_',targetRxn,'_d_',num2str(dist),'.pdf']);
%% calculate the coexpression and plot the correlation
contrRxns = glycolysisSet(:,1);
...
plt.XLim = xlim1;
plt.YLim = ylim1;
plt.export(['figures/glycan_example/FPA_glycan_flux_glycolysis_PC1_',targetRxn,'.pdf']);
