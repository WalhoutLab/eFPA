function [FluxPotentials_2d,FluxPotential_solutions_2d] = FPA2_cluster_multiPara_eng_targetOut(model,targetRxns,master_expression,distMat,labels,nSeq,manualPenalty,manualDist,maxDist,blockList, constantPenalty,parforFlag,penalty_defined,base,alpha, distMat_unweighted,labels_unweighted, eng_suffix)
...
penalty_ori = penalty_new;
%% part2.5 prepare the distance matrix of the unweighted distance matrix
% prepare the distance matrix
fprintf('Preparing the unweighted distance matrix...\n');
% create the full distance matrix
maxDist3 = max(distMat_unweighted(~isinf(distMat_unweighted)));
distMat3 = maxDist3 * ones(length(model_irrev.rxns),length(model_irrev.rxns)); % default distance is the maximum distance
[A B] = ismember(model_irrev.rxns,labels_unweighted);
% filter all Inf values
distMat_unweighted(distMat_unweighted > maxDist) = maxDist;
distMat3(A,A) = distMat_unweighted(B(A),B(A)); % overlay the input distance matrix to default in case any distance is missing in the input
% update the distance matrix and label
labels_unweighted = model_irrev.rxns;
distMat_unweighted = distMat3;
...
if parforFlag
    for bInd = 1:length(base)
        ...
        for nInd = 1:length(nSeq)
            ...
            FluxPotentials = cell(length(targetRxns),size(penalty_ori,2));
            FluxPotential_solutions = cell(length(targetRxns),size(penalty_ori,2));
            n = nSeq(nInd);
            ...
            parfor i = 1:length(targetRxns)
                ...
                penalty = penalty_ori;
                % remove the target gene expression 
                if doForward
                    penalty(strcmp(labels, [myrxn,'_f']),:) = 1;
                end
                if doReverse
                    penalty(strcmp(labels, [myrxn,'_r']),:) = 1;
                end
                ...
                for j = 1:size(penalty,2)
                    ...
                    if doForward 
                        ...
                        FPproblems_f{j}.penalty = penalty(:,j);
                        FPproblems_f{j}.Dist = distMat(strcmp(labels, [myrxn,'_f']),:);
                        FPproblems_f{j}.pDist = pDist_f;
                        FPproblems_f{j}.labels = labels;
                        FPproblems_f{j}.objRxn = [myrxn,'_f'];
                    else
                        FPproblems_f{j} = {};
                    end
                    if doReverse 
                        ...
                        FPproblems_r{j}.penalty = penalty(:,j);
                        FPproblems_r{j}.Dist = distMat(strcmp(labels, [myrxn,'_r']),:);
                        FPproblems_r{j}.pDist = pDist_r;
                        FPproblems_r{j}.labels = labels;
                        FPproblems_r{j}.objRxn = [myrxn,'_r'];
                    else
                        FPproblems_r{j} = {};
                    end
                end
                ...
                for j = 1:size(penalty,2)
                    if ~isempty(allLP_sol{j})
                        ...
                        FPVector_plus{1,j}{1} = rmfield(FPVector_plus{1,j}{1},setdiff(fieldnames(FPVector_plus{1,j}{1}),'full'));
                        FPVector_plus{1,j}{1}.weight = allLP{j}.weight;
                        FPVector_plus{1,j}{1}.penalty = allLP{j}.penalty;
                        FPVector_plus{1,j}{1}.Dist = allLP{j}.Dist;
                        FPVector_plus{1,j}{1}.pDist = allLP{j}.pDist;
                        FPVector_plus{1,j}{1}.labels = allLP{j}.labels;
                        FPVector_plus{1,j}{1}.objRxn = allLP{j}.objRxn;
                        
                        % clean up the solution 
                        keep = FPVector_plus{1,j}{1}.full > 1e-9;
                        FPVector_plus{1,j}{1}.full = FPVector_plus{1,j}{1}.full(keep);
                        FPVector_plus{1,j}{1}.weight = FPVector_plus{1,j}{1}.weight(keep);
                        FPVector_plus{1,j}{1}.penalty = FPVector_plus{1,j}{1}.penalty(keep);
                        FPVector_plus{1,j}{1}.Dist = FPVector_plus{1,j}{1}.Dist(keep);
                        FPVector_plus{1,j}{1}.pDist = FPVector_plus{1,j}{1}.pDist(keep);
                        FPVector_plus{1,j}{1}.labels = FPVector_plus{1,j}{1}.labels(keep);
                        % insert the unweighted distance 
                        [A B] = ismember(FPVector_plus{1,j}{1}.labels,labels_unweighted);
                        FPVector_plus{1,j}{1}.UnwDist = distMat_unweighted(strcmp(labels_unweighted, FPVector_plus{1,j}{1}.objRxn),B(A));
                    else
                        FPVector{1,j}(1) = NaN;
                        % FPVector_plus{1,j}(1) = {''};
                    end
                    if ~isempty(allLP_sol{size(penalty,2) + j})
                        ...
                        FPVector_plus{1,j}{2} = rmfield(FPVector_plus{1,j}{2},setdiff(fieldnames(FPVector_plus{1,j}{2}),'full'));
                        FPVector_plus{1,j}{2}.weight = allLP{size(penalty,2)+j}.weight;
                        FPVector_plus{1,j}{2}.penalty = allLP{size(penalty,2)+j}.penalty;
                        FPVector_plus{1,j}{2}.Dist = allLP{size(penalty,2)+j}.Dist;
                        FPVector_plus{1,j}{2}.pDist = allLP{size(penalty,2)+j}.pDist;
                        FPVector_plus{1,j}{2}.labels = allLP{size(penalty,2)+j}.labels;
                        FPVector_plus{1,j}{2}.objRxn = allLP{size(penalty,2)+j}.objRxn;
                             
                        % clean up the solution 
                        keep = FPVector_plus{1,j}{2}.full > 1e-9;
                        FPVector_plus{1,j}{2}.full = FPVector_plus{1,j}{2}.full(keep);
                        FPVector_plus{1,j}{2}.weight = FPVector_plus{1,j}{2}.weight(keep);
                        FPVector_plus{1,j}{2}.penalty = FPVector_plus{1,j}{2}.penalty(keep);
                        FPVector_plus{1,j}{2}.Dist = FPVector_plus{1,j}{2}.Dist(keep);
                        FPVector_plus{1,j}{2}.pDist = FPVector_plus{1,j}{2}.pDist(keep);
                        FPVector_plus{1,j}{2}.labels = FPVector_plus{1,j}{2}.labels(keep);
                        % insert the unweighted distance 
                        [A B] = ismember(FPVector_plus{1,j}{2}.labels,labels_unweighted);
                        FPVector_plus{1,j}{2}.UnwDist = distMat_unweighted(strcmp(labels_unweighted, FPVector_plus{1,j}{2}.objRxn),B(A));
                    else
                        FPVector{1,j}(2) = NaN;
                        % FPVector_plus{1,j}(2) = {''};
                    end
            ...
            if length(nSeq) > 1
                ...
                % save out the solutions as files
                save([eng_suffix,'/base_',num2str(k_base),'_n_',num2str(n),'.mat'],'FluxPotential_solutions')
            else
