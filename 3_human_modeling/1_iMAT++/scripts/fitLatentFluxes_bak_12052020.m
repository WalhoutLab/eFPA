...
if any(strcmp(model.rxns,'EX_majorNutr'))
    fprintf('the total major nutrient constraint is updated to %.2f \n',MILProblem.ub(find(strcmp(model.rxns,'EX_majorNutr'))));
end
...
while 1
    ...
    if any(strcmp(model.rxns,'EX_majorNutr'))
        MILProblem.ub(find(strcmp(model.rxns,'EX_majorNutr'))) = solution.full(find(strcmp(model.rxns,'EX_majorNutr')))*(1+latentCAP);
        model.ub(find(strcmp(model.rxns,'EX_majorNutr'))) = solution.full(find(strcmp(model.rxns,'EX_majorNutr')))*(1+latentCAP);
        fprintf('the total major nutrient constraint is updated to %.2f \n',MILProblem.ub(find(strcmp(model.rxns,'EX_majorNutr'))));
    end
