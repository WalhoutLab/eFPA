function [OFD, PFD, N_highFit, N_zeroFit, minLow, minTotal, minTotal_OFD, MILP, MILP_PFD, HGenes, RLNames, OpenGene, latentRxn, Nfit_latent, wasteDW] = IMATplusplus(model,epsilon_f,epsilon_r,ExpCateg, modelType, speedMode,minLowTol,doMinPFD, doLatent,latentCAP,storeProp,SideProp,ATPm,majorCoef,verbose)
...
if (nargin < 14) 
    majorCoef = 1.1; % for ihuman only; how much bac uptake could be released to allow sufficient minimization of low flux
end
if (nargin < 15) 
...
elseif modelType == 3
    fprintf('Doing integration for ihuman model... \n');
...
if modelType == 3 && majorCoef ~= -1 % run the dynamic constriant for unlimited bacUp
    MILProblemTemp = MILProblem;
    MILProblemTemp.c = zeros(length(MILProblem.c),1);
    MILProblemTemp.c(find(strcmp(worm.rxns,'EX_majorNutr'))) = 1;
    MILProblemTemp.osense = 1;
    solution = solveCobraMILP_XL(MILProblemTemp, 'timeLimit', 7200, 'logFile', 'MILPlog', 'printLevel', verbose);
    fprintf('the minimal major nutrient demand is %f \n',solution.obj);
    % impose boundary constraint
    MILProblem.ub(find(strcmp(worm.rxns,'EX_majorNutr'))) = majorCoef * solution.obj;
    worm.ub(find(strcmp(worm.rxns,'EX_majorNutr'))) = majorCoef * solution.obj;
end
...
if doMinPFD
    ...
    % in addition, the major nutrient is allow increased with the cap
    if modelType == 3 && majorCoef ~= -1
        MILProblem2.ub(find(strcmp(worm.rxns,'EX_majorNutr'))) = solution.full(find(strcmp(worm.rxns,'EX_majorNutr')))*(1+latentCAP);
        worm.ub(find(strcmp(worm.rxns,'EX_majorNutr'))) = solution.full(find(strcmp(worm.rxns,'EX_majorNutr')))*(1+latentCAP);
    end
    MILP2 = MILProblem2; 
