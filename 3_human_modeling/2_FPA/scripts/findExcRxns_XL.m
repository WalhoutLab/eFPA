function [selExc, selUpt] = findExcRxns_XL(model, inclObjFlag, irrevFlag)
...
% XL 12262020 -- allow non-standard exchange rxns such as non-integer
% stoichemitry 
...
if (~irrevFlag)
    % Find exchange rxns
    selExc = full((sum(model.S<0,1) >=1) & (sum(model.S>0) == 0))' | full((sum(model.S>0,1) >=1) & (sum(model.S<0) == 0))';
