function [FluxPotentials,FluxPotential_solutions] = MET_FPA_clusterWrapper(model,targetRxns,master_expression,distMat,distMat_raw,labels,n,manualPenalty,manualDist,maxDist,blockList, helperMet, constantPenalty,parforFlag,penalty_defined,alpha)
...
save([tmpDir,'/variables.mat'],'model','master_expression','distMat','distMat_raw','labels','n','manualDist','maxDist','blockList', 'constantPenalty','parforFlag','penalty','targetRxns','environment','helperMet','alpha','-v7.3','-nocompression');
...
for i = 1:length(splits)-1
    ...
    cmd = ['matlab -nodisplay -nosplash -nojvm -r \"MET_FPA_clusterExecuter ',tmpDir,' ',num2str(splits(i)),' ',num2str(splits(i+1)-1),' ',batchID,'\"'];
...
batchID = ['batch_',num2str(i)];
env_cmd = 'module load gurobi/900 && module load matlab/R2019a && ';
cmd = ['matlab -nodisplay -nosplash -nojvm -r \"MET_FPA_clusterExecuter ',tmpDir,' ',num2str(splits(i)),' ',num2str(length(targetRxns)),' ',batchID,'\"'];
full_cmd = [env_cmd, cmd, ' > ',tmpDir,'/',batchID,'.log && wait'];
bsub_cmd = ['bsub -q short -W 1:00 -n 1 -R rusage[mem=50000] -e ',tmpDir,'/err_',batchID,'.log ','-J ',batchID,' "'];
cmd_ready = [bsub_cmd, full_cmd,'"'];
system(cmd_ready);
