function [FluxPotentials_all,FluxPotential_solutions_all] = FPA_minAllowance(model,targetRxns,master_expression,distMat,labels,nSeq,manualPenalty,manualDist,maxDist,blockList, constantPenalty,parforFlag,penalty_defined)
...
if parforFlag
    for nInd = 1:length(nSeq)
        ...
        parfor i = 1:length(targetRxns)
            ...
            % WHEN WE HAVE DIFFERENT CONSTRAINTS IN DIFFERENT CONDITIONS,
            % WE NEED TO CALCULATE THE MINALLOWANCE FOR EACH CONDITION
            % INDIVIDUALLY; 
            for j = 1:(size(penalty,2)) %loop over all the queried conditions
                % block the reactions in the block list
                model_irrev_tmp0 = model_irrev;
                if ~isempty(blockList)
                    if ~isempty(blockList{j})
                        [A B] = ismember(blockList{j}(:,1),model_irrev_tmp0.rxns);
                        model_irrev_tmp0.lb(B(A)) = cell2mat(blockList{j}(:,2));
                        model_irrev_tmp0.ub(B(A)) = cell2mat(blockList{j}(:,3));
                    end
                end
                
                % calculate the minimal flux allowance for the target reaction
                if doForward %calculate the minAllowance of the forward reaction              
                    ...
                    % when calculate the minAllowance, we assume maximum
                    % expression for all genes (use penalty for super cond)
                    ...
                    [minAllowance_f] = GetMinAllowance(model_irrev_tmp,w_f, labels, [myrxn,'_f'],1, 'max');
                    % fprintf('flux allowance (f) for %s is %f\n',targetRxns{i},minAllowance_f);
                else
                    minAllowance_f = NaN;
                end
                if doReverse 
                    ...
                    [minAllowance_r] = GetMinAllowance(model_irrev_tmp,w_r, labels, [myrxn,'_r'],1, 'max');
                    % fprintf('flux allowance (r) for %s is %f\n',targetRxns{i},minAllowance_r);
                else
                    minAllowance_r = NaN;
                end     
                
                % calculate the real FP
