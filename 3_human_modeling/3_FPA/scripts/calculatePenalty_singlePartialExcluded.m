function penalty = calculatePenalty_singlePartialExcluded(model,master_expression,manualPenalty)
...
parfor i = 1:length(master_expression)
    ...
    [levels(:,i), status(:,i)] = gene_to_reaction_levels_singlePartialExcluded(model, expression.genes, expression.value, @min, @(x,y)(x+y));%GPR parser to convert the expression of genes to levels of "AND" gated blocks
