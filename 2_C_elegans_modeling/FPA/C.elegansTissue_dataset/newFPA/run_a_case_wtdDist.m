function [relFP_f,relFP_r, FluxPotential_solutions_X,FluxPotential_solutions_I] = run_a_case_wtdDist(targetObj,isMet)
...
if ~isMet
    ...
    [FluxPotential_X,FluxPotential_solutions_X] = FPA_oriMERGE_eng(model,targetRxns_X,master_expression,distMat,labels,n, manualPenalty2,manualDist,maxDist,blockList,constantPenalty2);
end
...
if ~isMet
    ...
    [FluxPotential_I, FluxPotential_solutions_I] = FPA_oriMERGE_eng(model,targetRxns_I,master_expression,distMat,labels,n, manualPenalty,manualDist,maxDist,blockList_I,constantPenalty);
end
...
if isMet
    ...
    if contains(targetExRxns{i},'DMN') || contains(targetExRxns{i},'SNK') % demand reaction; note in the dual model, 'SNK' is actually demand, the uptake direction is replaced by 'UPK' reactions
        ...
        [FluxPotential_X_met(i,:),FluxPotential_solutions_X_met(i,:)] = FPA_oriMERGE_eng(model_tmp,{[targetExRxns{i},'_X']},master_expression,distMat,labels,n, manualPenalty2,manualDist,maxDist,blockList,constantPenalty2,false,penalty_defined_X);
        % then, for intestine
        model_tmp = model;
        ...
        [FluxPotential_I_met(i,:),FluxPotential_solutions_I_met(i,:)] = FPA_oriMERGE_eng(model_tmp,{[targetExRxns{i},'_I']},master_expression,distMat,labels,n, manualPenalty,manualDist,maxDist,blockList_I,constantPenalty,false,penalty_defined_I);
    ...
    elseif contains(targetExRxns{i},'UPK') % sink is called 'UPK' in dual model
        ...
        model_tmp.ub(ismember(model_tmp.rxns,myRxns_X)) = 0;
        model_tmp.lb(ismember(model_tmp.rxns,myRxns_X)) = 0;
        [FluxPotential_X_met(i,:),FluxPotential_solutions_X_met(i,:)] = FPA_oriMERGE_eng(model_tmp,{[targetExRxns{i},'_X']},master_expression,distMat,labels,n, manualPenalty2,manualDist_tmp_X,maxDist,blockList,constantPenalty2,false,penalty_defined_X);
        model_tmp = model;
        myRxns_I = myRxns(cellfun(@(x) ~isempty(regexp(x,'(_L|_I)$','once')),myRxns));
        model_tmp.ub(ismember(model_tmp.rxns,myRxns_I)) = 0;
        model_tmp.lb(ismember(model_tmp.rxns,myRxns_I)) = 0;
        [FluxPotential_I_met(i,:),FluxPotential_solutions_I_met(i,:)] = FPA_oriMERGE_eng(model_tmp,{[targetExRxns{i},'_I']},master_expression,distMat,labels,n, manualPenalty,manualDist_tmp_I,maxDist,blockList_I,constantPenalty,false,penalty_defined_I);
