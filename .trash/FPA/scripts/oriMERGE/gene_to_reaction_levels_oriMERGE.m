function [reaction_levels, reaction_status] = gene_to_reaction_levels_oriMERGE( model, genes, levels, f_and, f_or )
...
% the output now is a string of expression levels for "AND"-connected blocks
    ...
    preParsedGrRules = preparseGPR_xl_oriMERGE(model.grRules);
    for i = 1:length(model.rxns)
        [level,status] = eval_gpr_oriMERGE(preParsedGrRules{i}, genes, levels, f_and, f_or);
