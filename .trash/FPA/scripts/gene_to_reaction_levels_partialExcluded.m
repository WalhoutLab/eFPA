function [reaction_levels, reaction_status] = gene_to_reaction_levels_partialExcluded( model, genes, levels, f_and, f_or )
...
% the output now is a string of expression levels for "AND" connected blocks
    ...
    for i = 1:length(model.rxns)
        [level,status] = eval_gpr_partialExcluded(preParsedGrRules{i}, genes, levels, f_and, f_or);
