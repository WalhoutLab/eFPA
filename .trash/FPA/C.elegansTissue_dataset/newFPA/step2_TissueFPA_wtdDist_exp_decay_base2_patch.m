%% About
% when we run the large-scale FPA script in the cluster, we noticed that
% sometimes an optimization ran into numerical issues for unknown reason.
% it produced NaN in the prediction. We rerun these error places locally on
% a laptop as a patch for it.
...
% create the target rxns list to patch
load(['output/FPA_rxns_wtdDist_exp_decay_base2.mat']);
rFP = [relFP_f;relFP_r];
...
rFP(ind1,:) = [];
labels(ind1) = [];
% get the problematic calculations 
ind1 = any(isnan(rFP),2);
rxns_error = labels(ind1);
rxns_error = unique(regexprep(rxns_error,'_.$',''));

targetExRxns = rxns_error(cellfun(@(x) ~isempty(regexp(x,'^(UPK|TCE|SNK|DMN)','once')),rxns_error));
targetMets = rxns_error(cellfun(@(x) ~isempty(regexp(x,'\[.\]','once')),rxns_error)); % targets for metabolite-centric calculation
targetMets2 = cellfun(@(x) ['NewMet_',x],targetMets,'UniformOutput',false);
% merge Ex rxns with Met (give a marker)
targetExRxns = [targetExRxns;targetMets2];
targetRxns = setdiff(rxns_error,union(targetMets,targetExRxns));
%% prepare files
load('Tissue.mat');
...
distance_raw = readtable('input/distanceMatrix_weighted.txt','FileType','text','ReadRowNames',true);
...
penalty = calculatePenalty_oriMERGE(model,master_expression2,manualPenalty2);
...
[FluxPotential_X] = FPA2_oriMERGE_base2(model,targetRxns_X,master_expression,distMat,labels,n, manualPenalty2,manualDist,maxDist,blockList,constantPenalty2);
%% OPTIMIZE INTESTINE
...
penalty = calculatePenalty_oriMERGE(model,master_expression2,manualPenalty);
I_ind = contains(model.rxns,'_I');
...
[FluxPotential_I] = FPA2_oriMERGE_base2(model,targetRxns_I,master_expression,distMat,labels,n, manualPenalty,manualDist,maxDist,blockList_I,constantPenalty);
...
penalty_defined_X = calculatePenalty_oriMERGE(model,master_expression,manualPenalty2);
penalty_defined_I = calculatePenalty_oriMERGE(model,master_expression,manualPenalty);
...
parfor i = 1:length(targetExRxns)
    ...
    if contains(targetExRxns{i},'DMN') || contains(targetExRxns{i},'SNK') % demand reaction; note in the dual model, 'SNK' is actually demand, the uptake direction is replaced by 'UPK' reactions
        ...
        [FluxPotential_X_met(i,:),FluxPotential_solutions_X_met(i,:)] = FPA2_oriMERGE_base2(model_tmp,{[targetExRxns{i},'_X']},master_expression,distMat,labels,n, manualPenalty2,manualDist,maxDist,blockList,constantPenalty2,false,penalty_defined_X);
        % then, for intestine
        model_tmp = model;
        ...
        [FluxPotential_I_met(i,:),FluxPotential_solutions_I_met(i,:)] = FPA2_oriMERGE_base2(model_tmp,{[targetExRxns{i},'_I']},master_expression,distMat,labels,n, manualPenalty,manualDist,maxDist,blockList_I,constantPenalty,false,penalty_defined_I);
    ...
    elseif contains(targetExRxns{i},'UPK') % sink is called 'UPK' in dual model
        ...
        model_tmp.ub(ismember(model_tmp.rxns,myRxns_X)) = 0;
        model_tmp.lb(ismember(model_tmp.rxns,myRxns_X)) = 0;
        [FluxPotential_X_met(i,:),FluxPotential_solutions_X_met(i,:)] = FPA2_oriMERGE_base2(model_tmp,{[targetExRxns{i},'_X']},master_expression,distMat,labels,n, manualPenalty2,manualDist_tmp_X,maxDist,blockList,constantPenalty2,false,penalty_defined_X);
        model_tmp = model;
        myRxns_I = myRxns(cellfun(@(x) ~isempty(regexp(x,'(_L|_I)$','once')),myRxns));
        model_tmp.ub(ismember(model_tmp.rxns,myRxns_I)) = 0;
        model_tmp.lb(ismember(model_tmp.rxns,myRxns_I)) = 0;
        [FluxPotential_I_met(i,:),FluxPotential_solutions_I_met(i,:)] = FPA2_oriMERGE_base2(model_tmp,{[targetExRxns{i},'_I']},master_expression,distMat,labels,n, manualPenalty,manualDist_tmp_I,maxDist,blockList_I,constantPenalty,false,penalty_defined_I);
    ...
    elseif contains(targetExRxns{i},'TCE') % transporter reactions
        ...
        [FluxPotential_X_met_f,FluxPotential_solutions_X_met_f] = FPA2_oriMERGE_base2(model_tmp,{targetrxn_fullName},master_expression,distMat,labels,n, manualPenalty2,manualDist,maxDist,blockList,constantPenalty2,false,penalty_defined_X);
        ...
        [FluxPotential_I_met_f,FluxPotential_solutions_I_met_f] = FPA2_oriMERGE_base2(model_tmp,{targetrxn_fullName},master_expression,distMat,labels,n, manualPenalty,manualDist,maxDist,blockList_I,constantPenalty,false,penalty_defined_I);
        ...
        [FluxPotential_X_met_r,fluxEfficiency_plus_X_met_r] = FPA2_oriMERGE_base2(model_tmp,{targetrxn_fullName},master_expression,distMat,labels,n, manualPenalty2,manualDist,maxDist,blockList,constantPenalty2,false,penalty_defined_X);
        ...
        [FluxPotential_I_met_r,FluxPotential_solutions_I_met_r] = FPA2_oriMERGE_base2(model_tmp,{targetrxn_fullName},master_expression,distMat,labels,n, manualPenalty,manualDist,maxDist,blockList_I,constantPenalty,false,penalty_defined_I);
    ...
    elseif contains(targetExRxns{i},'NewMet_') % new demand reaction needed for metabolite in question
        ...
        [distMat_raw_2,labels_X_2] = updateDistance_oriMERGE(targetrxn_fullName,model_tmp,distMat_raw,labels,maxDist);
        ...
        penalty_defined_X_2 = calculatePenalty_oriMERGE(model_tmp,master_expression,manualPenalty2);% recaluclate the penalty mat because of adding reaction
        myRxns_X = myRxns(cellfun(@(x) ~isempty(regexp(x,'(_X)$','once')),myRxns));
        model_tmp.ub(ismember(model_tmp.rxns,myRxns_X)) = 0;
        model_tmp.lb(ismember(model_tmp.rxns,myRxns_X)) = 0;
        [FluxPotential_X_met(i,:),FluxPotential_solutions_X_met(i,:)] = FPA2_oriMERGE_base2(model_tmp,{targetrxn_fullName},master_expression,distMat_X_2,labels_X_2,n, manualPenalty2,manualDist,maxDist,blockList,constantPenalty2,false,penalty_defined_X_2);
        ...
        [distMat_raw_2,labels_I_2] = updateDistance_oriMERGE(targetrxn_fullName,model_tmp,distMat_raw,labels,maxDist);
        % take the min and block intestine
        ...
        penalty_defined_I_2 = calculatePenalty_oriMERGE(model_tmp,master_expression,manualPenalty);% recaluclate the penalty
        myRxns_I = myRxns(cellfun(@(x) ~isempty(regexp(x,'(_L|_I)$','once')),myRxns));
        model_tmp.ub(ismember(model_tmp.rxns,myRxns_I)) = 0;
        model_tmp.lb(ismember(model_tmp.rxns,myRxns_I)) = 0;
        [FluxPotential_I_met(i,:),FluxPotential_solutions_I_met(i,:)] = FPA2_oriMERGE_base2(model_tmp,{targetrxn_fullName},master_expression,distMat_I_2,labels_I_2,n, manualPenalty,manualDist,maxDist,blockList_I,constantPenalty,false,penalty_defined_I_2);
    else
...
for i = 1:size(FluxPotential_X,1)
    for j = 1:length(tissueLabel)
        if ~strcmp(tissueLabel{j},'Intestine')
            if FluxPotential_X{i,end}(1) ~= 0 && ~isnan(FluxPotential_X{i,end}(1))% fix a bug!
                relFP_f(i,j) = FluxPotential_X{i,j}(1) ./ FluxPotential_X{i,end}(1);
            else
                relFP_f(i,j) = 0;
            end
            if FluxPotential_X{i,end}(2) ~= 0 && ~isnan(FluxPotential_X{i,end}(2))
                relFP_r(i,j) = FluxPotential_X{i,j}(2) ./ FluxPotential_X{i,end}(2);
            else 
                relFP_r(i,j) = 0;
            end
        else
            if FluxPotential_I{i,end}(1) ~= 0 && ~isnan(FluxPotential_I{i,end}(1))
                relFP_f(i,j) = FluxPotential_I{i,j}(1) ./ FluxPotential_I{i,end}(1);
            else 
                relFP_f(i,j) = 0;
            end
            if FluxPotential_I{i,end}(2) ~= 0 && ~isnan(FluxPotential_I{i,end}(2))
...
FPAtbl = readtable('relativeFluxPotentials.tsv','FileType','text','ReadRowNames',true);% same as table S7 and S8
alltarget = unique(cellfun(@(x) x(1:end-1),FPAtbl.Properties.RowNames,'UniformOutput',false));
targetExRxns0 = alltarget(cellfun(@(x) ~isempty(regexp(x,'^(UPK|TCE|SNK|DMN)','once')),alltarget));
targetMets = alltarget(cellfun(@(x) ~isempty(regexp(x,'\[.\]','once')),alltarget)); % targets for metabolite-centric calculation
targetMets2 = cellfun(@(x) ['NewMet_',x],targetMets,'UniformOutput',false);
% merge Ex rxns with Met (give a marker)
targetExRxns0 = [targetExRxns0;targetMets2];
targetRxns0 = setdiff(alltarget,union(targetMets,targetExRxns0));
targetRxns0 = [targetRxns0; alltarget(cellfun(@(x) ~isempty(regexp(x,'^(UPK|TCE|SNK|DMN)','once')),alltarget));targetMets2];

targetRxns1 = [targetRxns; targetExRxns];
[A B] = ismember(targetRxns0,targetRxns1);
relFP_f1 = relFP_f;
relFP_r1 = relFP_r;
% create the target rxns list to patch
load(['output/FPA_rxns_wtdDist_exp_decay_base2.mat']);
relFP_f(A,:) = relFP_f1(B(A),:);
relFP_r(A,:) = relFP_r1(B(A),:);
save('output/FPA_rxns_wtdDist_exp_decay_base2.mat','relFP_f','relFP_r');
delete(gcp('nocreate'))
