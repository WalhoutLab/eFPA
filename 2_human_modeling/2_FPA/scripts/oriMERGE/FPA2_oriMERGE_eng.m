function [FluxPotentials,FluxPotential_solutions] = FPA2_oriMERGE_eng(model,targetRxns,master_expression,distMat,labels,n,manualPenalty,manualDist,maxDist,blockList, constantPenalty,parforFlag,penalty_defined)
...
if parforFlag
    ...
    penalty = calculatePenalty_oriMERGE(model,master_expression,manualPenalty);
else
    ...
    parfor i = 1:length(targetRxns)
        ...
        for j = 1:size(penalty,2)
            ...
            if doForward 
                ...
                [FPVector{1,j}(1),FPVector_plus{1,j}{1}] = solvePLP_oriMERGE(model_irrev_tmp,w_f, labels, [myrxn,'_f'],1, 'max');
                FPVector_plus{1,j}{1}.penalty = penalty(:,j);
                FPVector_plus{1,j}{1}.Dist = distMat(strcmp(labels, [myrxn,'_f']),:);
                FPVector_plus{1,j}{1}.pDist = pDist_f;
                FPVector_plus{1,j}{1}.labels = labels;
                FPVector_plus{1,j}{1}.rxns  = model_irrev_tmp.rxns;
            else
                FPVector{1,j}(1) = NaN;
                FPVector_plus{1,j}(1) = {''};
            end
            if doReverse 
                ...
                [FPVector{1,j}(2),FPVector_plus{1,j}{2}] = solvePLP_oriMERGE(model_irrev_tmp,w_r, labels, [myrxn,'_r'],1, 'max');
                FPVector_plus{1,j}{2}.penalty = penalty(:,j);
                FPVector_plus{1,j}{2}.Dist = distMat(strcmp(labels, [myrxn,'_r']),:);
                FPVector_plus{1,j}{2}.pDist = pDist_r;
                FPVector_plus{1,j}{2}.labels = labels;
                FPVector_plus{1,j}{2}.rxns  = model_irrev_tmp.rxns;
            else
                FPVector{1,j}(2) = NaN;
                FPVector_plus{1,j}(2) = {''};
            end
