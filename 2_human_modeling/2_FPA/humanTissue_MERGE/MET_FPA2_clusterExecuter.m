function MET_FPA2_clusterExecuter(tmpDir,i,j,batchID)
addpath(genpath('~/cobratoolbox/'));
addpath ./input/
addpath ./../scripts/
addpath ./../scripts/oriMERGE/
addpath ./../../bins/
load([tmpDir,'/variables.mat'],'model','master_expression','distMat_raw','distMat','labels','n','maxDist','blockList','constantPenalty','manualDist','penalty','targetRxns','environment','k_base','helperMet','alpha');
...
for i = 1:length(targetExRxns)
    if contains(targetExRxns{i},'NewMet_') % new demand reaction needed for metabolite in question
        ...
        [FluxPotential_met_f] = FPA2_cluster(model_tmp,{targetrxn_fullName},master_expression,distMat_2,labels_2,n, {},manualDist,maxDist,blockList,constantPenalty,false,penalty_defined_2,k_base,alpha);   
        ...
        model_tmp.ub(ismember(model_tmp.rxns,myRxns)) = 0;
        % model_tmp.lb(ismember(model_tmp.rxns,myRxns)) = 0;
        [FluxPotential_met_r] = FPA2_cluster(model_tmp,{targetrxn_fullName},master_expression,distMat_2,labels_2,n, {},manualDist,maxDist,blockList,constantPenalty,false,penalty_defined_2,k_base,alpha);
        ...
        [FluxPotential_met_f] = FPA2_cluster(model_tmp,{targetrxn_fullName},master_expression,distMat,labels,n, {},manualDist,maxDist,blockList,constantPenalty,false,penalty,k_base,alpha);
        ...
        [FluxPotential_met_r] = FPA2_cluster(model_tmp,{targetrxn_fullName},master_expression,distMat,labels,n, {},manualDist,maxDist,blockList,constantPenalty,false,penalty,k_base,alpha);
        %% merge the potentials
        FluxPotential_tmp = cell(1,size(penalty,2));
        for z = 1:length(FluxPotential_tmp)
            FluxPotential_tmp{1,z} = [FluxPotential_met_f{z}(1),FluxPotential_met_r{z}(2)];
