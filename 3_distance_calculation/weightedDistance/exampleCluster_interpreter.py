...
def makeRxnDistanceMatrix(Folder='Output/Reactions/',saveTo='Output/distanceMatrix'):
    ...
    A1=np.inf*np.ones((N,N));
    A2=np.inf*np.ones((N,N));
    Lnotfound=[];
    ...
    for rxn in Lrxn:
        ...
        if goOn:
            ...
            for torxn in Dpath:
                Lpath=Dpath[torxn][0];
                distWtd = Dpath[torxn][1];
                if not Lpath is None:
                    A1[DI[rxn],DI[torxn]]=len(Lpath)-1;
                    A2[DI[rxn],DI[torxn]]=distWtd;
    print str(np.sum(Lbad))+' reactions were at the end of questionable paths. Check the returned list.';
    print 'Saving from-to matrix';
    writeMatrix2File(A1,DI,DI,saveTo+'_plain.txt');
    writeMatrix2File(A2,DI,DI,saveTo+'_weighted.txt');
