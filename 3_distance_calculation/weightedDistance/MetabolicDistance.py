...
class simplenode():
    def __init__(self,rxnid,parent,children,path,wtdDist):
        '''
        ...
        self.wtdDist = wtdDist; # this reflects the weighted distace to the current node ('rxnid')
...
class rxnnode():
    def __init__(self,rxnid,fromThese,toThese):
        ...
        self.toThese_weightedDist = [0] *  self.toNo
        self.fromThese_weightedDist = [0] *  self.fromNo


    def addParent(self,rxn,fromThese_weightedDist):
        if type(rxn)==str:
            rxnid=rxn;
        else:
            rxnid=rxn.id;
        ...
        self.fromNo=len(self.fromThese);
        self.fromThese_weightedDist.append(fromThese_weightedDist);

    def addChild(self,rxn,toThese_weightedDist):
        if type(rxn)==str:
            rxnid=rxn;
        else:
            rxnid=rxn.id;
        ...
        self.toNo=len(self.toThese);   
        self.toThese_weightedDist.append(toThese_weightedDist);
            
    def removeParent(self,rxnid):
        if self.fromThese_ids.count(rxnid):
            L1=[x for x in self.fromThese];
            L2=[x for x in self.fromThese_weightedDist];
            self.fromThese=[];
            self.fromThese_ids=[];
            for i in range(len(L1)):
                ...
                if not rxnid==xid:
                    ...
                    self.fromThese_weightedDist.append(L2[i]);
            self.fromNo=len(self.fromThese);
        else:
    ...
    def removeChild(self,rxnid):
        if self.toThese_ids.count(rxnid):
            L1=[x for x in self.toThese];
            L2=[x for x in self.toThese_weightedDist];
            self.toThese=[];
            self.toThese_ids=[];
            self.toThese_weightedDist = [];
            for i in range(len(L1)):
                x = L1[i];
                if not type(x)==str:
                    xid=x.id;
                ...
                if not rxnid==xid:
                    ...
                    self.toThese_weightedDist.append(L2[i]);
            self.toNo=len(self.toThese);
        else:
...
class rxnnetwork():
    ...
    def __init__(self,Drxn2reactants,Drxn2products,Dcpd2asreactant,Dcpd2asproduct):
        ...
        The weighted distance is defined as (D/4)^k, where D is the minimal degree of the connecting metabolite(s) and k is the scaling coefficient
        we use k = 1 to start the test
        the degree is calculated on the irreversible network, so that the linear reversible reaction pair will get 1, linear irreversible will get 0.5
        '''
        self.Drxn2node={};
        ...
        for rxn in self.Drxn2node:      # add the weighted distance attribute 
            for i in range(self.Drxn2node[rxn].toNo):
                self.Drxn2node[rxn].toThese_weightedDist[i] = min([float(len(Dcpd2asreactant[cpd])+len(Dcpd2asproduct[cpd]))/4 for cpd in set(Drxn2reactants[self.Drxn2node[rxn].toThese[i]]).intersection(Drxn2products[rxn])])                            
        for rxn in self.Drxn2node:      # add the weighted distance attribute 
            for i in range(self.Drxn2node[rxn].fromNo):
                self.Drxn2node[rxn].fromThese_weightedDist[i] = min([float(len(Dcpd2asreactant[cpd])+len(Dcpd2asproduct[cpd]))/4 for cpd in set(Drxn2products[self.Drxn2node[rxn].fromThese[i]]).intersection(Drxn2reactants[rxn])])                            
    ...
    def pathForward(self,rxn):
        ...
        tentativeDist = 0;
        currMinWtdDist = 0;
        topnode=simplenode(rxn,None,self.Drxn2node[rxn].toThese_ids,[],0);
        Dfirst={};
        self.updateDfirst(topnode,currMinWtdDist,Dfirst,0);
        while currMinWtdDist >= 0:
            #print currMinWtdDist
            newparents=[];
            for x in Dfirst:
                if Dfirst[x][1]==currMinWtdDist:
                    newparents.append(Dfirst[x][0]);
            for pa in newparents:
                #print 'parent '+pa.id
                for i in range(pa.childno):
                    if not type(pa.children[i])==str:
                        childid=pa.children[i].id;
                    else:
                        childid=pa.children[i];
                    #print childid;
                    # calculate the accumulated weighted distance
                    minWeightedDist_acc = self.Drxn2node[pa.id].toThese_weightedDist[i] + pa.wtdDist;
                    grandchildren=self.Drxn2node[childid].toThese_ids;
                    pa.children[i]=simplenode(childid,pa,grandchildren,pa.path,minWeightedDist_acc); #update the children from rxnID to simplenode
                    if not grandchildren:
                        terminals.append(pa.children[i].id); 
                    self.updateDfirst(pa.children[i],currMinWtdDist,Dfirst,minWeightedDist_acc);
            tentativeDist=[];
            for x in Dfirst:
                if Dfirst[x][1]>currMinWtdDist:
                    tentativeDist.append(Dfirst[x][1]);
            if tentativeDist == []:
                currMinWtdDist = -1;
            else:
                currMinWtdDist = min(tentativeDist);
    ...
    def updateDfirst(self,node,currMinWtdDist,Dfirst,minWeightedDist):
        #now, we keep the shortest path with minimal minWeightDist
        '''Subroutine of tree function (e.g., self.pathForward()).'''
        if not Dfirst.has_key(node.id):
            Dfirst[node.id]=(node,minWeightedDist,node.path);
        else:
            #if claim mimimal, then skip
            if Dfirst[node.id][1] > currMinWtdDist:
                #check if a shorter weighted path is found 
                if minWeightedDist < Dfirst[node.id][1]:
                    #if checkIfDuplicates(node.path): #if there is dup, it is an invalid path!
    ...
    def getPaths(self,direction='forward'):
        ...
        for k in getattr(self,'Dfirst_'+direction):
            Dpath[k]=[getattr(self,'Dfirst_'+direction)[k][2],getattr(self,'Dfirst_'+direction)[k][1]];
        return Dpath
    ...
    def replaceRemoved(self,Lrxn=None):
        ...
        for rxn in Lrxn:
            ...
            for i in range(len(self.Drxn2node[rxn].fromThese)) :
                x=self.Drxn2node[rxn].fromThese[i]
                y=float(self.Drxn2node[rxn].fromThese_weightedDist[i]);
                x.addChild(self.Drxn2node[rxn],y);
            for i in range(len(self.Drxn2node[rxn].toThese)):
                x=self.Drxn2node[rxn].toThese[i]
                y=self.Drxn2node[rxn].toThese_weightedDist[i]
                x.addParent(self.Drxn2node[rxn],y);            
    ...
    for looprxn in Llooprxns:
        ...
        while path and isLoop and k<kmax:
            ...
            if Net.Dfirst_forward.has_key(looprxn):
                path=Net.Dfirst_forward[looprxn][2];
                distWtd = Net.Dfirst_forward[looprxn][1];
            else:
                path=None;
                distWtd = None;
            if path:
